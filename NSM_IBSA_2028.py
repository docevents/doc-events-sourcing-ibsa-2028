...
<style>
    .doc-header {
        background-color: #FBBD1E; padding: 18px 32px;
        border-bottom: 4px solid #000000; margin-bottom: 24px; border-radius: 6px;
    }
    .doc-header h1 { color: #000000; margin: 0; font-size: 28px; font-family: Lato, sans-serif; }
    .doc-header p  { color: #000000; margin: 4px 0 0 0; font-size: 14px; }
    .section-title {
    ...
    [data-testid="metric-container"] { font-size: 14px !important; }
    [data-testid="metric-container"] div:first-child { font-size: 12px !important; }
    [data-testid="metric-container"] div:last-child { font-size: 18px !important; }
</style>
...
# ── File upload ───────────────────────────────────────────────────────────────
# Auto-load NSM_IBSA_2028.xlsx

try:
    with st.spinner("Loading event data…"):
        df = pd.read_excel("NSM_IBSA_2028.xlsx", sheet_name=0, header=None)
except FileNotFoundError:
    st.error("❌ Event file not found. Please ensure NSM_IBSA_2028.xlsx is in the app directory.")
    st.stop()

# Set default event details (editable in P2)
p1_event_name = "NSM_IBSA_2028"
p1_client_name = "IBSA Pharma"
p1_attendees = 50
p1_nights = 3
p1_meeting_days = 2
