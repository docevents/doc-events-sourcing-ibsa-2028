...
st.set_page_config(page_title="Doc Events – Sourcing", layout="wide")
...
for h in filtered:
    ...
    with c_card:
        with st.container(border=True):
            ...
            if h.get("website") and h["website"].lower() not in ["nan", ""]:
                name_col.link_button("🌐 Website", h["website"])
            ...
            # Rate display ($ amounts bigger and green)
            if not h["has_rate"]:
                ...
                if h["is_per_person"]:
                    ...
                    m2.markdown(f"**Resort Fee**<br><span style='font-size:20px;color:green;font-weight:bold'>${h['resort_fee']:.0f}/night</span>" if h["resort_fee"] > 0 else f"**Resort Fee**<br><span style='font-size:20px;color:green;font-weight:bold'>—</span>", unsafe_allow_html=True)
                if h["is_range"]:
                    m3.markdown(f"**Est. Accommodation**<br><span style='font-size:20px;color:green;font-weight:bold'>${h['est_cost']:,.0f} – ${h['est_cost_hi']:,.0f}</span>", unsafe_allow_html=True)
                    st.info(f"ℹ️ Rate range: **{h['rate_display']}** ({h['rate_source']}). "
                            f"Estimated accommodation shown for low and high end of range.")
                else:
                    m3.markdown(f"**Est. Accommodation**<br><span style='font-size:20px;color:green;font-weight:bold'>${h['est_cost']:,.0f}</span>", unsafe_allow_html=True)
                # Per-person note
                if h["is_per_person"]:
